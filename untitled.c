...
void AddBSTNode(BNODE **HeadNode, char MovieTheaterName[], char ZipCode[], char FileName[], char Dimensions[])
{
	BNODE *current_node;
	if (*HeadNode == NULL)
	{
	...
	{
	//	printf("ZipCode is %s HeadZIPCODE is %s \n", ZipCode,(*HeadNode)->ZipCode );
		int result = strcmp(ZipCode , (*HeadNode)->ZipCode);
	//	printf("String cmp result is %d\n", result);
		if (result < 0  )
		{
			//printf("Move left\n");
			AddBSTNode(&(*HeadNode)->left, MovieTheaterName,ZipCode,FileName,Dimensions);
		}
			
		else if(result >0  )
		{
			//printf("Move right\n\n");
			AddBSTNode(&(*HeadNode)->right,MovieTheaterName,ZipCode,FileName,Dimensions);
